/*
 * htool_ble.c - Fixed for OLED Menu indices (0-4)
 */
#include "htool_ble.h"
...
uint8_t microsoft_adv[15] = {0x0E, 0xFF, 0x06, 0x00, 0x03, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static char generate_random_char() {
    ...
    if (random_num < 10) return (char)('0' + random_num);
    else if (random_num < 36) return (char)('A' + (random_num - 10));
    else return (char)('a' + (random_num - 36));
}

// Logic adjusted for menu index 0-4
// 0=Apple, 1=Google, 2=Samsung, 3=Microsoft, 4=Random
void htool_ble_set_adv_data(uint8_t index) {
    // If Random (4) selected, pick one of 0-3
    if (index == 4) {
        index = esp_random() % 4;
    }
    ...
    if (index == 0) { // Apple
        int r = esp_random() % 29;
        esp_ble_gap_config_adv_data_raw((uint8_t *)apple_adv[r], r > 16 ? 23 : 31);
    }
    else if (index == 1) { // Google
        google_adv[13] = (esp_random() % 120) - 100; // Randomize byte
        esp_ble_gap_config_adv_data_raw((uint8_t *)google_adv, 14);
    }
    else if (index == 2) { // Samsung
        samsung_adv[14] = (esp_random() % 32) + 1; // Randomize byte
        esp_ble_gap_config_adv_data_raw((uint8_t *)samsung_adv, 15);
    }
    else if (index == 3) { // Microsoft
        // Randomize the payload (bytes 7-14)
        for (int i = 7; i <= 14; i++) {
            microsoft_adv[i] = generate_random_char();
        }
...
uint8_t htool_ble_deinit() {
    if (ble_init) {
        if (esp_bluedroid_disable() != ESP_OK) return HTOOL_BLE_ERR;
        if (esp_bluedroid_deinit() != ESP_OK) return HTOOL_BLE_ERR;
        if (esp_bt_controller_disable() != ESP_OK) return HTOOL_BLE_ERR;
        if (esp_bt_controller_deinit() != ESP_OK) return HTOOL_BLE_ERR;
        ble_init = false;
    }
    return HTOOL_BLE_OK;
}
...
uint8_t htool_ble_init() {
    if (!ble_init) {
        ...
        if (esp_bt_controller_init(&bt_cfg) != ESP_OK) return HTOOL_BLE_ERR;
        if (esp_bt_controller_enable(ESP_BT_MODE_BLE) != ESP_OK) return HTOOL_BLE_ERR;
        if (esp_bluedroid_init() != ESP_OK) return HTOOL_BLE_ERR;
        if (esp_bluedroid_enable() != ESP_OK) return HTOOL_BLE_ERR;
        if (esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_ADV, ESP_PWR_LVL_P9) != ESP_OK) return HTOOL_BLE_ERR;
        ble_init = true;
    }
    return HTOOL_BLE_OK;
}